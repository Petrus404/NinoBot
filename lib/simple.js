...
exports.WAConnection = _WAConnection => {
	class WAConnection extends _WAConnection {
		constructor(...args) {
			...
			this.browserDescription = ["NINO-BOT", "Edge", "6.9"]
			// Alias
			this.sendFileFromUrl = this.sendFileFromURL = this.sendFile
		}

		async sendSticker(jid, media, m, opts = {}) {
			let convert = require('./media').sticker
			let medias = await this.getFile(media)
			return this.sendMessage(jid, await convert(medias.data, null, opts.packname || global.packname, opts.author || global.author), 'stickerMessage', {
				quoted: m,
				...opts
			})
		}
